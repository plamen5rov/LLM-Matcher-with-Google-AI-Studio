      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-8 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10">
        ...
        <div className="lg:col-span-8 space-y-8">
          <section className="border border-[#141414] min-h-[600px] flex flex-col rounded-sm">
            <div className="border-b border-[#141414] p-4 relative flex items-center justify-between bg-white">
              ...
              <div className="flex items-center gap-2">
                {recommendations.length > 0 && (
                  <button 
                    onClick={shareSetup}
                    className={`p-2 rounded-lg transition-all flex items-center gap-2 text-xs font-bold ${showShareSuccess ? 'bg-green-500 text-white' : 'hover:bg-[#F5F5F7] text-[#1D1D1F] opacity-60 hover:opacity-100'}`}
                  >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <button 
                  onClick={onCopy}
                  className={`p-1.5 rounded-lg transition-all ${isCopied ? 'bg-green-500 text-white' : 'hover:bg-[#F5F5F7] text-[#86868B]'}`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {tools.map((tool) => (
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="bg-white p-6 rounded-[2rem] border border-[#D2D2D7]/30 shadow-sm hover:shadow-md transition-all group"
          >
            <div className="w-10 h-10 rounded-xl bg-[#F5F5F7] flex items-center justify-center mb-4 text-[#1D1D1F] group-hover:bg-[#1D1D1F] group-hover:text-white transition-colors">
              {tool.icon}
            </div>
            <h4 className="font-bold text-[#1D1D1F] mb-1">{tool.name}</h4>
            <p className="text-xs text-[#86868B] leading-relaxed">{tool.desc}</p>
          </a>
        ))}
      </div>
      
      <div className="p-6 bg-[#1D1D1F] rounded-[2rem] text-white flex flex-col sm:flex-row items-center justify-between gap-6">
